    ...
    conn.query(`INSERT INTO botapp(app_name,api_key,api_secrete,access_token,access_secret) VALUES('${appName}','${appKey}','${keySecrete}','${accessToken}','${accessSecrete}')`, 
        function (error, results) {
            ...
            if(results.affectedRows){
                res.json({code: 1,message:"App added successfully!",appid:results.insertId});
                res.end();
            }
            else{
                res.json({code: 0,message:"Insertion failed!"});
                res.end();
            }
        });
});
...
twiter_route.post('/addTag',async (req,res)=>{
    ...
    function (error, results) {
        if(error) throw error;
        if(results.affectedRows){
            res.json({code: 1,message:"Tag added successfully!",tag_id:results.insertId});
            res.end();
        }
        else{
            res.json({code: 0,message:"Insertion failed!"});
...
twiter_route.post('/addRetweet',async (req,res)=>{
    ...
    function (error, results) {
        if(error) throw error;
        if(results.affectedRows){
            checktags.updateTags();
            res.json({code: 1,message:"Retweet schedule is set successfully!"});
            res.end();
        }
        else{
            res.json({code: 0,message:"Insertion failed!"});
...
twiter_route.post('/getTable',async (req,res)=>{
    ...
    function (error, results) {
        ...
        if(results.length){
            ...
            res.json({code: 1,message:"Retweet schedule is set successfully!",list:rtac});
            res.end();
        }
        else{
...
twiter_route.post('/deleteTag',async (req,res)=>{
    ...
    function (error, results) {
        if(error) throw error;
        if(results.affectedRows){
            res.json({code: 1,message:"Retweet deleted from schedule successfully!"});
            res.end();
        }
        else{
            res.json({code: 0,message:"Insertion failed!"});
